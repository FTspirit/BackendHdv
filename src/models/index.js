...
const sequelizeGofaProduct = new Sequelize(
  ...
  config.db.postgresUSER.password,
  {
    ...
    // logging: config.db.postgresUSER.logging,
    define: {
      dialectOptions: {
        ssl: true,
      },
...
fs.readdirSync(`${__dirname}/hdv`)
  .filter((file) => file.indexOf('.') !== 0 && file !== basename && file.slice(-3) === '.js')
  .forEach((file) => {
    // eslint-disable-next-line global-require, security/detect-non-literal-require, import/no-dynamic-require
    const model = require(path.join(`${__dirname}/hdv`, file))(sequelizeGofaProduct, Sequelize.DataTypes);
    db[model.name] = model;
  });
